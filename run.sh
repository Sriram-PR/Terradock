...
if [ ! "$(ls -A /data/style/)" ]; then
    mv /home/renderd/src/openstreetmap-carto-backup/* /data/style/
fi
...
if [ "$1" == "import" ]; then
    ...
    sudo -u postgres createuser renderd
    sudo -u postgres createdb -E UTF8 -O renderd gis
    sudo -u postgres psql -d gis -c "CREATE EXTENSION postgis;"
    sudo -u postgres psql -d gis -c "CREATE EXTENSION hstore;"
    sudo -u postgres psql -d gis -c "ALTER TABLE geometry_columns OWNER TO renderd;"
    sudo -u postgres psql -d gis -c "ALTER TABLE spatial_ref_sys OWNER TO renderd;"
    setPostgresPassword
    ...
    if [ "${UPDATES:-}" == "enabled" ] || [ "${UPDATES:-}" == "1" ]; then
        ...
        sudo -E -u renderd openstreetmap-tiles-update-expire.sh $REPLICATION_TIMESTAMP
    fi
    ...
    chown -R renderd: /home/renderd/src/ /data/style/
    if [ -f /data/style/scripts/get-external-data.py ] && [ -f /data/style/external-data.yml ]; then
        sudo -E -u renderd python3 /data/style/scripts/get-external-data.py -c /data/style/external-data.yml -D /data/style/data
    fi

    # Register that data has changed for mod_tile caching purposes
    sudo -u renderd touch /data/database/planet-import-complete
...
if [ "$1" == "run" ]; then
    ...
    if [ "${UPDATES:-}" == "enabled" ] || [ "${UPDATES:-}" == "1" ]; then
        ...
        sudo -u renderd touch /var/log/tiles/run.log; tail -f /var/log/tiles/run.log >> /proc/1/fd/1 &
        sudo -u renderd touch /var/log/tiles/osmosis.log; tail -f /var/log/tiles/osmosis.log >> /proc/1/fd/1 &
        sudo -u renderd touch /var/log/tiles/expiry.log; tail -f /var/log/tiles/expiry.log >> /proc/1/fd/1 &
        sudo -u renderd touch /var/log/tiles/osm2pgsql.log; tail -f /var/log/tiles/osm2pgsql.log >> /proc/1/fd/1 &
    fi
    ...
    sudo -u renderd renderd -f -c /etc/renderd.conf &
    child=$!
    wait "$child"
